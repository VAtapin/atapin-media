...
Кнопка ? в title bar каждой программы открывает отдельное окно справки поверх рабочего. Videos/Playlist явно различают сохранённую локальную структуру, локальные описания, физически доступный video original и внешний переход на YouTube. Сохранённые описания/картинки не выдаются за наличие видеозаписи. История/отмена AI для SourceRecords, редактор импортированных playlist positions и полный адаптер Takeout остаются следующими этапами.
...
Failed/partial/cancelled imports можно повторить кнопкой в истории. Сохраняются источник, владелец, оригиналы и стабильный ID запуска; активный запуск повторно не ставится в очередь. История показывает prepare/download/extract/files/metadata, не выдуманный общий процент. Import anhalten отменяет queued сразу; running становится stop_requested и завершается cancelled на ближайшем checkpoint. Downloads имеют Process stop; hash/copy/текущий archive entry могут закончиться прежде остановки. Already accepted originals сохраняются, недописанные части не регистрируются как готовые; retry перечитывает сохранённую папку/архив, не восстанавливает каждый processing cursor. Защищён claim задания и обработан worker timeout/failure. Реальный yt-dlp stop требует серверной проверки.
