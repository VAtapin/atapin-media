...
Единая папка `/var/www/vhosts/mannavomhimmel.de/private/Takeout` автоматически появляется как **Bereits entpackter Export: Takeout**. Для другого deployment путь можно задать через `TAKEOUT_FOLDER`. Она регистрируется на месте на отдельном private disk `takeout-prepared`; прежний `TAKEOUT_ROOT`/disk `takeout` для ZIP и ранее импортированных папок не меняется. Весь `private` не сканируется и перенос originals не требуется.
