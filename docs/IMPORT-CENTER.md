...
Правило владельца для Manna Vom Himmel: ролики примерно на 15 секунд обычно являются **Beiträge**, а не Videos/Shorts. ИИ получает длительность из metadata материала/связанного оригинала и installation-specific правило из `config/import_classification.php` (14–16 секунд, включительно). Правило передаётся как доверенная инструкция отдельно от импортных текстов, не действует на другие бренды и не распространяется на все короткие видео. Без длительности или при противоречиях нельзя угадывать; один лишь хронометраж не запускает платный запрос без текстовых/визуальных данных. Существующие Takeout-защиты остаются: рекомендация `posts` сохраняется в журнале ИИ, изменение экспортного типа Takeout подтверждается редактором. Массовое переназначение и новые ИИ-задания при импорте не запускаются.
