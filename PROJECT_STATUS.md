...
- Для Media Library ещё не реализованы native list/grid UI, защищённые inline previews, запуск AI-классификации, собственный resumable upload и отключение временного `/upload/` после production-проверки новой загрузки.
...
- Реализовать native list/grid-интерфейс Media Library на расширенной модели, затем подключить preview/download, AI-классификацию и собственную resumable upload-передачу.
...
- `Expand media library foundation`
