...
- Для Media Library ещё не реализованы grid-представление, защищённые inline previews, запуск AI-классификации, собственный resumable upload и отключение временного `/upload/` после production-проверки новой загрузки.
...
- Подключить защищённые inline previews для поддерживаемых форматов, затем реализовать AI-классификацию и собственную resumable upload-передачу в Media Library.
