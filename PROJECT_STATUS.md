...
- Live: обновляемый модерируемый чат и счётчик активных browser sessions. Live Studio — единственный административный интерфейс вещания и работает через JSON API внутри `/desktop`; отдельный legacy `/desktop/live` HTML-интерфейс удалён. Для события в Live Studio можно загрузить одно защищённое изображение-афишу через существующую Media-библиотеку; оно связывается как `cover_media_id` и используется публичными Live-карточками/анонсом. Будущие события не попадают в «Aufzeichnungen»: записи отделены фильтром `live_status=ended`, а Live Studio сортирует текущие/запланированные события выше завершённых.
...
- Последняя реализация: Keep one OBS ingest address across livestream events.
