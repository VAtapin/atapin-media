...
- Отдельное окно Browser Studio показывает беззвучную локальную копию canvas, без второго публичного плеера или отдельной передачи. После закрытия окна встроенное превью восстанавливается; кнопка повторного открытия остаётся доступной в заголовке. Поле WebRTC hostname подставляет hostname текущей страницы, если серверное значение пусто, и объясняет требуемый формат. Применение защищённой серверной конфигурации показывает ход, отказ и результат непосредственно возле кнопки; при откате указывает этап отказа. Успешно сохранённая Browser-настройка глобальна для следующих Livestreams.
...
- На production владелец проверил существующую конфигурацию MediaMTX: `api=false`, `webrtc=false`, локальный API `127.0.0.1:9997` не слушает и отвечает `HTTP 000`, хотя фоновый процесс держит server lock. Попытка включить Browser-Übertragung откатилась с прежним общим сообщением; настройка не сохранялась. Существующий MediaMTX-конфиг валиден, FFmpeg имеет `libx264`/AAC, приватная папка и конфиг принадлежат пользователю Plesk; точный этап отказа ещё не установлен. Публичный TCP/UDP 8189 открыт владельцем, но API не включается открытием firewall-порта. Реальная трансляция не подтверждена.
...
- После обновления кода без активной трансляции повторить защищённое применение Browser-конфигурации в Live Studio и получить указанный возле кнопки точный этап отказа либо подтверждение сохранения. При отказе устранить соответствующую серверную причину; после успеха проверить локальный API и пройти реальный smoke-test трансляции. Публичные API/WHIP/RTSP порты открывать не требуется.
...
- Текущий функциональный блок: этот commit — `Expose Live server configuration failures`. Branch/upstream: `main` → `origin/main`.
- Предыдущий функциональный commit: `0739c20` — `Expand Browser Studio after preview pop-out`.
