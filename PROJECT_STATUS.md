...
- Live: обновляемый модерируемый чат и счётчик активных browser sessions. Live Studio — единственный административный интерфейс вещания и работает через JSON API внутри `/desktop`; отдельный legacy `/desktop/live` HTML-интерфейс удалён. ИИ-помощник использует существующий ключ/provider/model, отдельный запрос с согласием, приватный ответ, опубликованный текстовый контекст и общий дневной лимит вызовов; включается владельцем, не отвечает автоматически на каждое сообщение.
...
- Последняя реализация: Keep MediaMTX HLS cookie-check inside the public proxy.
