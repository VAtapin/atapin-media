...
- Для Desktop подключены полные наборы из 19 approved PNG для Manna Vom Himmel и Standard. В системных настройках выбираются набор значков, фон и акцентный цвет без изменения ярлыков, окон или их расположения.
...
- Наборы значков описаны в `platform/config/desktop.php`; новый клиентский набор добавляется как запись конфигурации и папка с теми же 19 именами файлов.
...
- Пройден `node --check` для `desktop-os.js` и `desktop-shortcuts.js`; проверено наличие и парность 19 PNG в каждом из двух наборов.
...
- `Configure desktop shortcuts and appearance`
