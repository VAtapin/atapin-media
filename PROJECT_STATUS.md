...
- Полный сценарий Takeout/каталога: восемь частей обрабатываются совместно; немецкие Videos/Videotexte CSV, Beiträge с точными image attachments/опросами, comments и playlist positions становятся цельными SourceRecords/Collections. IDs объединяют прежние записи без замены ручных текстов; global SHA/bytes/MIME registry используется и для обычных/resumable uploads, alias locations сохранены. Пообъектный журнал имеет added/merged/duplicate/linked/unsupported/failed/ambiguous/unmatched, pagination/filter/retry; completed extraction/file/metadata checkpoints переживают stop/retry. Два почти одинаковых названия в реальном экспорте различаются точными пробелами: read-only проверка сопоставила 244/244 видео, без распаковки 55 GB.
- Media Library по умолчанию показывает цельные материалы; отдельный файловый просмотр начинается с thumbnail gallery. ИИ классифицирует файлы, но не создаёт из них самостоятельные Beiträge/видео, и не меняет исходный kind Takeout-записей. Takeout не запускает автоматические paid AI jobs. Старый YouTube-каталог и image-generated Beiträge можно убрать из активного каталога с сохранёнными обратимыми записями; originals/ZIP/собственные статьи/фото/документы не удаляются. Restore не перезаписывает новые совпадающие IDs. Новые Takeout-записи исключены из повторной очистки старого каталога.
- Видимая кнопка Lokale Videos prüfen есть в Media Library, Import Center и Videos: очередь проверяет все registered videos, original locations, bytes/ffprobe и отсутствующие assets у описаний. Report отличает server available/failed/missing от browser not_checked/playable/failed. Alle im Browser prüfen делает same-origin HTTP Range, декодирование кадра, seek и реальную короткую playback-пробу, сохраняет результат и поддерживает stop. Protected local preview теперь имеет HTTP 206/seek; при утрате canonical location доступны сохранённые identical alternate originals. Связанный commit — текущий `Rebuild complete Takeout content and verify local videos`.

- Временный `/upload/` окончательно отключён: index всегда redirect `/desktop`, API всегда 410; UI app.js/app.css/login.php удалены. Private archive не изменяется, прежний инструмент доступен в Git. CI сохраняет archive/auth проверки и проверяет retired HTTP contract вместо старой загрузки. Связанный commit — `f99ef67 Retire the temporary upload interface`.
...
- Публичный YouTube-архив сохранён в private/manna-youtube. Приватный Google Takeout импортируется отдельным адаптером из private/youtube_zip_alle: группы ZIP распознаются по export ID/номеру части, отдельный ZIP отчёта исключается. Все части проверяются до распаковки; originals остаются вне public root.
- Временный /upload/ окончательно отключён (f99ef67): redirect на Desktop, API 410; старые UI-файлы удалены, приватный intake archive сохранён. Отключение применяется после получения кода на production.
...
- Семь пунктов Media Library и пять пунктов Import Center реализованы в коде; серверная приёмка реальных originals ещё не выполнялась. Новый Takeout-адаптер проверен на немецкой CSV-схеме владельца; произвольная другая структура/язык экспорта не обещается поддержанной. Неизвестные schemas остаются originals и видны в отчёте. ffprobe требуется для server format check, browser playback проверяется отдельно без ИИ. Полная распаковка 55 GB/production import не запускались.
- Public Website начат, но ещё не завершён.
- Resume сохраняет completed ZIP entries/files/metadata checkpoints, пропускает повторное hash/copy неизменённых завершённых файлов. Текущий ZIP entry/hash может закончиться или начаться снова после interruption; ZIP не выдаётся за побайтовый resumable download. Старые дубли оригиналов не удаляются. Takeout не содержит отдельного достоверного Short-признака: сохраняются ранее известные Shorts; новые видео без него имеют format_needs_review, не угадываются по имени.
...
- После deployment: Import Center → Alten Katalog zurücksetzen (preview/counts, originals сохраняются) → Google Takeout vom Server → выбрать takeout-20260911T193951Z-1 и 8 частей → Import starten → Ergebnis im Detail. Затем Lokale Videos prüfen → Alle im Browser prüfen. При нехватке private storage импорт блокируется до распаковки.

## Проверки

- Takeout/каталог/playback: 42 целевых Laravel tests / 251 assertions прошли; после последней правки CSV/image connections повторены только 5 Takeout tests / 27 assertions. PHP/JS syntax и реальный GET Desktop прошли. Edge Import Center workflow (desktop/mobile, upload receipts, gallery/content, playlists, help) прошёл. Отдельный браузерный workflow подтвердил на синтетическом локальном WebM HTTP 206, decoded frame, seek, короткое фактическое воспроизведение и сохранение browser_status; mobile dialog помещается. Полный suite, платные provider calls, реальный 55 GB import и playback серверных originals не запускались.
