...
- Live: обновляемый модерируемый чат и счётчик активных browser sessions. Гостю явно предлагается войти перед отправкой сообщения, а пользователю с правом модерации показана ссылка на очередь проверки `/desktop/community/moderation`; опубликованный чат обновляется через heartbeat. Live Studio — единственный административный интерфейс вещания и работает через JSON API внутри `/desktop`; отдельный legacy `/desktop/live` HTML-интерфейс удалён. Для события в Live Studio можно загрузить одно защищённое изображение-афишу через существующую Media-библиотеку; оно связывается как `cover_media_id` и используется публичными Live-карточками/анонсом. Будущие события не попадают в «Aufzeichnungen»: записи отделены фильтром `live_status=ended`, а Live Studio сортирует текущие/запланированные события выше завершённых.
...
- Для Live Studio локально прошли `node --check` для изменённых JavaScript-файлов и `git diff --check`; целевые Feature-тесты покрывают общий ingest, poster Live-события, отображение данных следующего эфира и разделение scheduled/ended, но PHP/Laravel Feature tests недоступны на Windows, потому что PHP не установлен. Для direct RTMPS локально выполнены только read-only diff checks; MediaMTX binary validation на Windows недоступна.
...
- Последняя реализация: Present public Live event details and moderation clearly.
