...
- Media Library ещё требует управления коллекциями, массовой разметки/фильтрации, archive/restore UI, карточки связей/использования, просмотра и отмены AI-истории и локального технического processing. Import Center ещё требует полного адаптера личных YouTube exports, межисточниковой дедупликации, upload pause/cancel/folder/queue, управления долгими импортами и более подробного результата. Реальный личный архив нужен для подтверждения полноты адаптера; серверная приёмка не заменяется локальными тестами.
...
- Текущий этап: Simplify imports and explain local content in desktop windows (commit с этой записью).
