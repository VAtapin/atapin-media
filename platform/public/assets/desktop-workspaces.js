(() => {
  ...
  const rows=(root,items,select)=>{const list=root.querySelector('[data-workspace-list]');list.replaceChildren();if(!items.length)list.append(el('p',t('empty'),'workspace-empty'));for(const row of items){const b=el('button',undefined,'workspace-row');b.type='button';b.append(el('strong',row.title||row.name||row.subject||row.question||row.email||`#${row.id}`),el('small',[t(row.status||row.kind||''),row.refresh_status?t(row.refresh_status):'',[row.due_date?.slice(0,10),row.due_time].filter(Boolean).join(' '),row.project?.title,row.assignee?.name,row.source].filter(Boolean).join(' · ')));b.addEventListener('click',()=>run(root,()=>select(row)));list.append(b);}};
