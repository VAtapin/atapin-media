(() => {
  ...
  const crud = (root, config) => {
    ...
    else { filters.append(field('q','search')); if (config.statuses) filters.append(field('status','select','',[['',t('all')], ...config.statuses])); for (const f of config.filters || []) { const label = field(...f); filters.append(label); if (config.filterLookups?.[f[0]]) lookup(label, config.filterLookups[f[0]]).catch(error => feedback(root, error.message, true)); } filters.append(button('refresh', () => load(1))); const cardsButton = button('book_view_cards', () => { view = 'cards'; try { localStorage.setItem(viewKey, view); } catch (_) {} renderItems(); cardsButton.setAttribute('aria-pressed','true'); listButton.setAttribute('aria-pressed','false'); }); const listButton = button('book_view_list', () => { view = 'list'; try { localStorage.setItem(viewKey, view); } catch (_) {} renderItems(); cardsButton.setAttribute('aria-pressed','false'); listButton.setAttribute('aria-pressed','true'); }); cardsButton.setAttribute('aria-pressed', String(view === 'cards')); listButton.setAttribute('aria-pressed', String(view === 'list')); actions.append(button(config.newLabel || 'new', () => config.create ? config.create(root, load) : openSeparate(root.dataset.workspace)), cardsButton, listButton, button('refresh', () => load())); root.querySelector('[data-workspace-editor]').replaceChildren(el('p', t('choose'))); document.addEventListener('desktop-media-changed', () => { if (root.isConnected) load(1); }); load(); }
