...
const css=document.createElement('link');css.rel='stylesheet';css.href='/assets/desktop-browser-studio.css?v=1';document.head.append(css);
...
export async function initialize(root,event){
  ...
  const input=(key,type='text')=>{const label=W.el('label',t(key)),field=document.createElement('input');field.type=type;field.dataset.studioField=key;label.append(field);controls.append(label);return field;};
  const select=key=>{const label=W.el('label',t(key)),field=document.createElement('select');field.dataset.studioField=key;label.append(field);controls.append(label);return field;};
  const state={busy:()=>Boolean(pc||session||recording||preparing),dispose:()=>{
    ...
    recorder?.disconnect();sources.forEach(source=>source.disconnect());context?.close();imageUrls.forEach(url=>URL.revokeObjectURL(url));document.removeEventListener('click',guardClose,true);panel.remove();tabs.remove();grid.hidden=false;delete root.dataset.studioCapturing;
  ...
  try{
    ...
    const meter=document.createElement('meter');meter.min=0;meter.max=1;meter.setAttribute('aria-label',t('microphone'));controls.append(meter);
    ...
    controls.append(action('prepare',prepare),action('screen',async()=>{
    ...
    }),action('screen_stop',()=>{screen?.getTracks().forEach(track=>track.stop());screen=null;screenVideo.srcObject=null;if(scene==='screen'){scene=images.length?'image':'camera';scenes.value=scene;}}),action('mute',()=>{micVolume.value=Number(micVolume.value)>0?'0':'.8';micVolume.oninput();if(!pc)status.textContent=t(Number(micVolume.value)>0?'ready':'muted');}));
    ...
    controls.append(action('record',async()=>{if(!mic||!recorder||recording)throw new Error(t('choose_source'));if(wav&&!confirm(t('download')+'?'))return;wav=null;uploadedMediaId=null;pcm=[];pcmBytes=0;await context.resume();recording=true;recorder.port.postMessage(true);status.textContent=t('recording');}),action('record_stop',finishRecording),action('download',()=>{
    ...
    if(typeof window.uploadDesktopMedia==='function')controls.append(action('upload',async()=>{
    ...
    controls.append(action('podcast_draft',async()=>{if(!uploadedMediaId)throw new Error(t('audio_unavailable'));await request(endpoint+'/events/'+event.id+'/podcast',{method:'POST',body:JSON.stringify({confirm:true,media_id:uploadedMediaId})});status.textContent=t('podcast_created');}));
    ...
    controls.append(action('start',async()=>{
