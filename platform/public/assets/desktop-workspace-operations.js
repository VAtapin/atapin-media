(() => {
  ...
  W.register('analytics',root=>{
    ...
    const dateFrom=value=>{const date=new Date(`${String(value).slice(0,10)}T12:00:00`);return Number.isNaN(date.getTime())?null:date;};
    const addDays=(date,days)=>{const next=new Date(date.getTime());next.setDate(next.getDate()+days);return next;};
    const monday=date=>addDays(date,-((date.getDay()+6)%7));
    const completeDaily=(from,to,rows)=>{const known=new Map((rows||[]).map(row=>[String(row.occurred_on).slice(0,10),number(row.count)])),output=[];if(!from||!to)return rows||[];for(let date=new Date(from.getTime());date<=to;date=addDays(date,1)){const key=dateString(date);output.push({occurred_on:key,count:known.get(key)||0});}return output;};
    ...
    const renderChart=(card,rows)=>{
      ...
      const smoothPath=points=>{if(points.length<2)return `M ${points[0].x} ${points[0].y}`;let path=`M ${points[0].x} ${points[0].y}`;for(let index=0;index<points.length-1;index++){const previous=points[index-1]||points[index],current=points[index],next=points[index+1],following=points[index+2]||next,controlOne={x:current.x+(next.x-previous.x)/6,y:current.y+(next.y-previous.y)/6},controlTwo={x:next.x-(following.x-current.x)/6,y:next.y-(following.y-current.y)/6};path+=` C ${controlOne.x} ${controlOne.y}, ${controlTwo.x} ${controlTwo.y}, ${next.x} ${next.y}`;}return path;};
      const linePath=smoothPath(points),lastPoint=points[points.length-1],areaPath=`${linePath} L ${lastPoint.x} ${pad.top+innerHeight} L ${points[0].x} ${pad.top+innerHeight} Z`,area=document.createElementNS('http://www.w3.org/2000/svg','path');area.setAttribute('d',areaPath);area.setAttribute('class','analytics-chart-area');svg.append(area);
    ...
    const renderHeatmap=(card,rows)=>{
      const wrap=el('div',undefined,'analytics-heatmap-wrap'),labels=el('div',undefined,'analytics-heatmap-days'),grid=el('div',undefined,'analytics-heatmap-grid'),max=Math.max(1,...rows.map(row=>number(row.count))),weeks=Math.ceil(rows.length/7);
      for(let index=0;index<7;index++){const day=addDays(new Date(2024,0,1,12),index);labels.append(el('span',day.toLocaleDateString(document.documentElement.lang||'de-DE',{weekday:'short'})));}
      grid.style.gridTemplateColumns=`repeat(${weeks},minmax(16px,1fr))`;rows.forEach(row=>{const level=number(row.count)?Math.max(1,Math.ceil(number(row.count)/max*4)):0,cell=el('button',undefined,`analytics-heat-cell level-${level}`);cell.type='button';cell.setAttribute('aria-label',`${row.occurred_on}: ${formatNumber(row.count)} ${t('events').toLowerCase()}`);cell.dataset.tooltip=`${row.occurred_on} · ${formatNumber(row.count)}`;cell.addEventListener('click',()=>showDetail(row.occurred_on,`${t('daily')}: ${row.occurred_on}`,['occurred_on','count'],[row]));grid.append(cell);});
      wrap.append(labels,grid);const legend=el('div',undefined,'analytics-heatmap-legend');legend.append(el('span',t('less')),el('i',''),el('i',''),el('i',''),el('i',''),el('span',t('more')));card.append(wrap,legend);
    };
    const renderDashboard=(d,query)=>{
      const list=root.querySelector('[data-workspace-list]');list.replaceChildren();hideDetail();
      const events=d.events||[],daily=completeDaily(dateFrom(d.start),dateFrom(d.end),d.daily||[]),pages=d.pages||[],content=d.content||[],sales=d.summary.sales||[],eventTotal=events.reduce((total,row)=>total+number(row.count),0),saleTotal=sales.map(row=>formatCurrency(row.cents,row.currency)).join(' · ')||'—',saleCount=sales.reduce((total,row)=>total+number(row.count),0);
      ...
      const heatmapCard=section(t('activity_map'),t('click_for_details'),'analytics-card analytics-card-heatmap'),heatmapStart=monday(dateFrom(d.start)||new Date()),heatmapEnd=addDays(monday(dateFrom(d.end)||new Date()),6);renderHeatmap(heatmapCard,completeDaily(heatmapStart,heatmapEnd,daily));dashboard.append(heatmapCard);
