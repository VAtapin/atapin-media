    ...
    private function youtube():void
    {
        foreach(glob($this->root.'/items/*/metadata.json')?:[] as $file) {
            ...
            $this->attempt($id,function()use($id){
                ...
                $record = app(ContentMetadataImporter::class)->record('youtube',$id,in_array('shorts',$raw['sources']??[],true)?'short':'video',$info['title']??$id,$info['description']??'',
                    ['archive_path'=>'items/'.$id,'sources'=>$raw['sources']??[],'media'=>$mediaIds,'target_profile'=>$this->run->target_profile?:'videos','import_id'=>$this->run->id,
        ...
        foreach(glob($this->root.'/posts/*/post.json')?:[] as $file){
            ...
            $this->attempt($id,function()use($id){
                ...
                $record = app(ContentMetadataImporter::class)->record('youtube',$id,'post',mb_substr($post['text']??$id,0,120),$post['text']??'',
                    ['archive_path'=>'posts/'.$id,'published_label'=>$post['published_label']??null,'url'=>$post['url']??null,'images'=>$images,'target_profile'=>$this->run->target_profile?:'posts',
                        'likes_label'=>$post['likes_label']??null,'links'=>$post['links']??[],'raw'=>$post,'import_id'=>$this->run->id]);
