...
class BrowserBroadcast
{
    ...
    public function eligible(SourceRecord $record): bool
    {
        return !$record->trashed()&&$record->status==='ready'&&($record->metadata['public_section']??null)==='live'
            &&!empty($record->metadata['live_stream_enabled'])&&!empty($record->metadata['public_published']);
    }
    public function begin(User $user,SourceRecord $record,string $sdp): array
    {
        abort_unless($this->eligible($record),422,__('live-browser.browser_event_required'));
        ...
        $session=Cache::lock('live-input-owner',15)->block(3,function()use($user,$record){
            ...
            return LiveBrowserSession::create(['id'=>(string)Str::uuid(),'user_id'=>$user->id,'source_record_id'=>$record->id,'expires_at'=>now()->addMinutes(2)]);
        });
        try{
        ...
        }catch(\Throwable){$session->update(['status'=>'failed','expires_at'=>now()]);abort(503,__('live-browser.browser_connection_failed'));}
    }
