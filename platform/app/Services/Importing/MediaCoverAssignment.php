...
class MediaCoverAssignment
{
    public function assign(Media $image, SourceRecord $record): void
    {
        DB::transaction(function () use ($image, $record) {
            ...
            abort_unless(in_array($record->kind, ['video', 'short','post'], true), 422, __('imports.cover_video_required'));
            $meta = $record->metadata ?? [];
            $primary = Media::whereIn('id', app(ImportedContentPresentation::class)->mediaIds($meta))->where('kind', 'video')->first();
            abort_if($record->kind === 'post' && !$primary, 422, __('imports.cover_video_required'));
            $sharedDerivative = ($image->metadata['derivative'] ?? false) && $image->parent_id && $primary && $image->parent_id !== $primary->id;
            abort_if($primary && $image->parent_id && $image->parent_id !== $primary->id && !$sharedDerivative, 409, __('imports.cover_parent_conflict'));
            ...
            $image->update(['asset_role' => 'thumbnail', 'parent_id' => $sharedDerivative ? $image->parent_id : ($primary?->id ?? $image->parent_id)]);
