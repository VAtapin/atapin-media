    ...
    public function change(SourceRecord $record,array $data): void
    {
        DB::transaction(function()use($record,$data){
            ...
            if($chosen){
                ...
                abort_if($role==='audio' && ($chosen->kind!=='audio'||($record->metadata['public_section']??null)!=='podcast'),422,__('imports.asset_wrong_kind'));
            ...
            if($action==='replace' && $role==='audio')$remove=Media::whereIn('id',$ids)->where('kind','audio')->pluck('id')->all();
            ...
            foreach(['cover_media_id','primary_video_id','primary_audio_id'] as $key)if(in_array($metadata[$key]??null,$remove,true))unset($metadata[$key]);
            ...
            if($chosen){
                ...
                if($role==='audio'){$metadata['primary_audio_id']=$chosen->id;$metadata['media']['audio']=[$chosen->id];}
