...
class ContentEditorData
{
    ...
    {
        return ['workflow_stage'=>'sometimes|in:idea,script,production,review,approved','slug'=>'nullable|string|max:180|regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/','locale'=>'nullable|in:de,en','episode_number'=>'nullable|integer|min:1|max:100000','season'=>'nullable|integer|min:1|max:1000','podcast_format'=>'sometimes|in:audio,video','body_format'=>'sometimes|in:plain,html','platform_metadata'=>'sometimes|array:youtube,facebook,instagram,telegram,x',
    ...
    public static function metadata(array $metadata, array $data): array
    {
        ...
        foreach (['workflow_stage','slug','locale','episode_number','season','podcast_format','body_format','platform_metadata','author','seo_title','seo_description','transcript','guest','external_podcast_url','cover_media_id'] as $key) if (array_key_exists($key,$data)) $metadata[$key] = $data[$key];
