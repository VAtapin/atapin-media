...
class PublicAccountController extends Controller {
    ...
    public function index(Request $request,PublicContent $content,PublicBooks $books){
        ...
        $reviews=BookReview::with('product')->where('user_id',$user->id)->latest()->paginate(10,['*'],'reviews_page')->through(fn($review)=>['review'=>$review,'card'=>$review->product?$books->card($review->product):null,'statusLabel'=>$this->accountStatus($review->status,'review')]);
