...
@if($view==='board')<div class="task-board">@foreach($columns as $state=>$column)<section class="task-column"><h2>{{ __('ui.task_'.$state) }} <small>{{ $column->count() }}{{ $column->count()===50?'+':'' }}</small></h2>@forelse($column as $task)@include('tasks.card')@empty<p class="muted">{{ __('ui.no_tasks') }}</p>@endforelse@if($column->count()===50)<a href="{{ route('tasks.index',['view'=>'list','status'=>$state]) }}">{{ __('ui.view_all') }}</a>@endif</section>@endforeach</div>
