@php($iconOnly=$iconOnly??false)@if($subject)<form action="{{ route($subject instanceof \App\Models\Product?'public.book-state':'public.record-state',$subject) }}" method="post" data-public-form>@csrf<input type="hidden" name="action" value="{{ $action }}">
