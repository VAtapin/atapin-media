...
@elseif($section==='kontakt' && !empty($contactEmail))@if($orderBook??null)<h2>{{ __('public.order_context') }}: {{ $orderBook->title }}</h2>@endif<a href="mailto:{{ $contactEmail }}?subject={{ rawurlencode(($orderBook??null)?__('public.order_context').': '.$orderBook->title:__('public.contact')) }}">{{ $contactEmail }}</a>
