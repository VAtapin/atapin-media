...
@section('content')
@php($card=$featured)@php($liveStatus=$record?->metadata['live_status']??null)@php($liveDate=($record?->metadata['starts_at']??null)?\Illuminate\Support\Carbon::parse($record->metadata['starts_at'])->timezone(config('app.timezone'))->format('d.m.Y H:i'):null)@php($liveStatusLabel=$liveStatus?__('public.live_'.$liveStatus):null)@php($viewer=auth()->user())@php($sessionId=request()->hasSession()?request()->session()->getId():null)
