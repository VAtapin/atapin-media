<section class="desktop-workspace" data-workspace="{{ $app }}" data-can-publish="{{ auth()->user()->hasPermission('content.publish')?'true':'false' }}">
    <header class="workspace-head"><div><p class="workspace-eyebrow">{{ __('workspaces.eyebrow') }}</p><h1>{{ __('workspaces.titles.'.$app) }}</h1><p>{{ __('workspaces.intros.'.$app) }}</p></div><div data-workspace-actions></div></header>
