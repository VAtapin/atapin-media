<section class="desktop-community" data-community-moderation>
    ...
    <section class="desktop-community-queue">
        ...
        @forelse($entries as $entry)
            ...
            @php($adminReasons = app(\App\Services\PublicCommunityModeration::class)->adminReasons($moderation['reasons'] ?? []))
            <article class="desktop-community-entry">
                <header><div>@if($entry->kind !== 'live_chat')<h3>{{ $entry->title }}</h3>@endif<p>{{ $entry->metadata['author'] ?? __('public.guest') }} · {{ $entry->created_at->format('d.m.Y H:i') }} · {{ $entry->kind === 'live_chat' ? __('public.live_chat') : __('public.comments') }}</p></div><span class="desktop-community-risk">{{ __('ui.community_risk') }} {{ number_format((float)($moderation['risk'] ?? 1), 2, ',', '') }}</span></header>
                <div class="desktop-community-body">{{ $entry->body }}</div>
                @if($adminReasons)<ul class="desktop-community-reasons">@foreach($adminReasons as $reason)<li>{{ $reason }}</li>@endforeach</ul>@endif
