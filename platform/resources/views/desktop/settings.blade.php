    ...
    <div class="desktop-settings-workspace">
        ...
        <section class="desktop-settings-panel" data-settings-panel="profile" hidden>
            ...
            <form class="desktop-settings-form" data-profile-form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data">@csrf @method('PATCH')
                <div class="desktop-settings-profile-header"><img data-profile-avatar src="{{ $profile?->avatar_path ? route('profile.avatar').'?v='.$profile->updated_at->timestamp : '/assets/brand/owner/logo-mark.png' }}" alt=""><label>Profilfoto<input name="avatar" type="file" accept="image/png,image/jpeg,image/webp" @can('media.upload') data-auto-media-upload data-upload-target="avatar_media_id" @endcan data-image-upload-profile="avatar"><small data-image-upload-status>PNG, JPEG oder WebP. Wird @can('media.upload') sofort hochgeladen und @endcan automatisch auf Avatar-Größe optimiert.</small></label></div>
        ...
        <section class="desktop-settings-panel" data-settings-panel="desktop_design">
            ...
            <form class="desktop-settings-form" method="post" action="{{ route('settings') }}" enctype="multipart/form-data">
                ...
                <label class="desktop-settings-upload">{{ __('ui.desktop_custom_wallpaper') }}<input name="desktop_custom_wallpaper" type="file" accept="image/png,image/jpeg,image/webp" data-auto-media-upload data-image-upload-profile="wallpaper" data-upload-target="desktop_custom_wallpaper_media_id"><small data-image-upload-status>{{ __('ui.desktop_custom_wallpaper_hint') }} Wird sofort hochgeladen und automatisch auf maximal 2560 × 1440 und 3 MB optimiert.</small></label>
