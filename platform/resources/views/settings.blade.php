...
@section('content')
<div class="page-heading"><div><p class="eyebrow">{{ config('platform.brand') }}</p><h1>{{ __('ui.settings') }}</h1></div></div><section class="panel form-panel"><h2>{{ __('ui.website') }}</h2><form method="post" action="{{ route('settings') }}" enctype="multipart/form-data">@csrf @method('PUT')
...
<label>{{ __('ui.wallpaper') }}<select name="desktop_wallpaper">@foreach(config('desktop.wallpapers') as $key => $wallpaper)<option value="{{ $key }}" @selected(old('desktop_wallpaper',$settings['desktop_wallpaper'] ?? 'mountains') === $key)>{{ __('ui.'.$wallpaper['label_key']) }}</option>@endforeach<option value="custom" @selected(old('desktop_wallpaper',$settings['desktop_wallpaper'] ?? 'mountains') === 'custom')>{{ __('ui.desktop_wallpaper_custom') }}</option></select></label>
...
</div><label>{{ __('ui.desktop_custom_wallpaper') }}<input name="desktop_custom_wallpaper" type="file" accept="image/png,image/jpeg,image/webp"><small class="muted">{{ __('ui.desktop_custom_wallpaper_hint') }}</small></label><button class="button">{{ __('ui.save') }}</button></form></section>
@endsection
