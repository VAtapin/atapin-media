...
try{
  ...
  const live=page.locator('.os-window[data-app-id=live-studio]'),studio=live.locator('[data-browser-studio]');await live.locator('[data-live-help]').click();
  const help=page.locator('.os-window[data-app-id=help-live-studio]');await help.locator('.desktop-help').filter({hasText:'Für OBS'}).waitFor();assert.match(await help.textContent(),/Direkt im Browser senden/);await help.locator('[data-window-action=close]').click();
  await live.locator('[data-mode=browser]').click();await studio.locator('[data-studio-action=prepare]').waitFor();
  ...
  assert.deepEqual(errors,[]);assert.deepEqual(failures,[]);console.log('Live help plus real MediaMTX WHIP/WebRTC, local camera/microphone, image/PiP/title composition, WAV recording/download/library upload, busy-event protection and explicit stop passed desktop/mobile. Linux transcoding/Plesk/firewall were not exercised.');
