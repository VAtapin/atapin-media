...
try {
  ...
  await settingsFrame.locator('[data-settings-tab="users"]').click();
  const userEdit = settingsFrame.locator('[data-user-edit]').first();
  await userEdit.click();
  const userEditForm = settingsFrame.locator('[data-user-edit-form]:not([hidden])');
  const originalUserName = await userEditForm.locator('[name=name]').inputValue();
  await userEditForm.locator('[name=name]').fill('Nicht speichern');
  await userEditForm.locator('[data-user-cancel]').click();
  assert.equal(await userEditForm.isHidden(), true);
  await userEdit.click();
  assert.equal(await userEditForm.locator('[name=name]').inputValue(), originalUserName);
  await userEditForm.locator('[data-settings-save]').click();
  await settingsFrame.locator('[data-settings-notice]').getByText('Gespeichert.').waitFor();
  assert.equal(await userEditForm.isHidden(), true);
  await settingsFrame.locator('[data-user-create]').click();
  const userCreateForm = settingsFrame.locator('[data-user-create-form]');
  await userCreateForm.locator('[name=name]').fill('Nicht speichern');
  await userCreateForm.locator('[data-user-create-cancel]').click();
  assert.equal(await userCreateForm.isHidden(), true);
  assert.equal(await userCreateForm.locator('[name=name]').inputValue(), '');
  await settingsFrame.locator('[data-settings-tab="desktop_design"]').click();
  ...
  await page.locator('[data-start-button]').click();
  await page.locator('.os-start-menu [data-open-app="topics"]').click();
  assert.equal(await page.locator('.os-window[data-app-id="topics"]').getAttribute('data-snap-zone'),'3','Merged region did not reserve both cells');
  await page.reload();
  ...
  for(const appId of ['videos','tasks','topics'])await page.locator(`.os-window[data-app-id="${appId}"] [data-window-action="close"]`).click();
  ...
  console.log('Browser: desktop windows, direct settings, persistence and responsive layout OK');
